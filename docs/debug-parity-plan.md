...
> - **Attach to Process:已完成(2026-08-02)**。缺的是 `IDebugProgramProvider2`——沒有它,shell 在任何自訂程式碼執行前就找不到可附加的 program 而放棄(這就是 `0x8971001E`)。解法照 JavaScript/TypeScript adapter(唯一做本機 attach 的 in-box DAH engine):`Attach=1` + `AutoSelectPriority` + `PortSupplier` **單值** + `ProgramProvider` + `AlwaysLoadProgramProviderLocal`,並實作 `GoProgramProvider`/`GoProgramNode`(以 Go build-info magic 過濾,只對真正的 Go 行程回報)。協定記錄實證:`{"command":"attach","mode":"local","processId":N}` → `success:true`,Break All 產生 `stopped(reason=pause)`,detach 後目標續跑。詳見 `pitfalls.md` 第 15 條。
>
> - ~~**Attach to Process:未完成(卡住,需要新資訊)**~~(以下為當時的診斷記錄,保留供參)。已完成的部分:`GoAdapterLauncher`(`IAdapterLauncher`,`UpdateLaunchOptions` 產生 `{"$debugServer":port,"request":"attach","mode":"local","processId":N}`)、`DelveServer` 抽出供 F5/attach 共用、COM CLSID 註冊。**卡點**:透過 Go engine attach 一律以 `HRESULT 0x8971001E` 失敗,且失敗發生在 **adapter launcher 被呼叫之前**——dlv 從未啟動、ActivityLog 無記錄、DAH 協定無流量。對照組:同一個行程用 **Native engine attach 成功**,證明目標與 attach 管線本身沒問題。已嘗試且皆無效的註冊變體:(1) `AdapterLauncher` metric 值、(2) `ExtensibilityObjects` 編號子鍵、(3) `PortSupplier` 由單值改為編號子鍵列表。
